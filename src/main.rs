extern crate rand;
use rand::distributions::{Beta, Distribution, Exp, Uniform};
use rand::Rng;
...
enum NewickNode {
    Root {
        left_child: Box<NewickNode>,
        right_child: Box<NewickNode>,
    },
    Node {
        left_child: Box<NewickNode>,
        right_child: Box<NewickNode>,
...
impl NewickNode {
    fn to_newick(&self, print_brlens: bool) -> String {
        match self {
            ...
            &NewickNode::Node {
                ...
                if print_brlens {
                    "(".to_string()
                        + &lc.to_newick(print_brlens)
                        + ","
                        + &rc.to_newick(print_brlens)
                        + "):"
                        + &format!("{:.4}", w)
                } else {
                    "(".to_string()
...
fn make_ultrametric(tree: NewickNode, left: f64) -> NewickNode {
    match tree {
        ...
        NewickNode::Node {
            ...
            new_node_weightless(
                make_ultrametric(*lc, new_left),
                make_ultrametric(*rc, new_left),
                new_w,
            )
        }
        NewickNode::Leaf {
...
fn main() {
    ...
    let no_print_brlens = matches.is_present("n") || matches.is_present("nobranchlengths");
    ...
    println!("brlens: {}", no_print_brlens);
    println!("ultrametric: {}", ultrametric);

    if no_print_brlens {
        println!("{}", t.to_newick(false));
    } else {
        println!("{}", t.to_newick(true));
    }
}
